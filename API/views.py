...
def createReport(request, pk):
    ...
    if serializer.is_valid():
        ...
        notify.send(
            ...
            verb='New report',
            description=f'Child {child.name} report updated to {serializer.data}'
        )
...
def createnotes(request,pk):
    ...
    if serializer.is_valid():
          ...
          notify.send(
            ...
            verb='New nots',
            description=f'Child {child.name} nots updated to {serializer.data}'
        )
...
def chosemeal(request, pk):
        ...
        if serializer.is_valid():
          ...
          notify.send(
            ...
            recipient=teacher.user,
            verb='New meal',
            description=f'Child {child.name} meal updated to {serializer.data}'
        )
...
def sendlocation(request, pk):
    ...
    if serializer.is_valid():
        ...
        notify.send(
            ...
            recipient=mother.user,
            verb='New Location',
            description=f'Child {child.name} location updated to {serializer.data}'
        )
...
@api_view(['GET'])
def notificationsmom(request,pk):
    mother = request.user.mother 
...
@api_view(['GET'])
def notificationsteacher(request,pk):
    teacher = request.user.teacher 
    teacher = Teacher.objects.get(id=pk) 
    notifications = Notification.objects.filter(recipient=teacher.user)
    serialized_notifications = [{"verb": n.verb, "description": n.description} for n in notifications]
    
    return Response(serialized_notifications)
#تمت الاضافة لجلب اسم الوجبة مع الصورة
@api_view(['GET'])
def getmeal(request):
